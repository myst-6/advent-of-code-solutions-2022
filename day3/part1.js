const input = `lDDWVvlVVQfDMlWjGJTRjQCgTGLCLj
...
const halves = lines.map(line => [line.slice(0, line.length >> 1), line.slice(line.length >> 1)]);
const common = halves.map(([first, second]) => {
...
const values = common.map(char => " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(char));
