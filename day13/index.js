...
//   const first = JSON.parse(lines[i * 3]);
//   const second = JSON.parse(lines[i * 3 + 1]);
...
// -1, 0, 1
// 1 = correct, 0 = undetermined, -1 = bad
function compare(a, b) {
  ...
  if (typeof a === "number" && typeof b === "number") {
    ...
    while (a.length > 0 || b.length > 0) {
      ...
      const left = a.shift(), right = b.shift();
...
const packets = lines.filter(x => x).map(x => JSON.parse(x));
packets.push([[2]], [[6]]);

for (let i=1; i<packets.length; i++) {
  for (let j=0; j<packets.length-i; j++) {
...
const idx1 = packets.findIndex(x => x.length === 1 && Array.isArray(x[0]) && x[0].length === 1 && x[0][0] === 2) + 1;
const idx2 = packets.findIndex(x => x.length === 1 && Array.isArray(x[0]) && x[0].length === 1 && x[0][0] === 6) + 1;
